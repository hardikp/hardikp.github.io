...
1. Trading pattern differ significantly based on the type of the security (stocks, ETFs, options, futures, currencies), liquidity, minimum price increment, whether there is an underlying (Futures, ETFs, options) and many other factors. While this makes it hard to write execution algorithm, it also impacts backtesting.